...
The original proposal uses some agent names not present in the ECHO collective. Here is the validated mapping:
...
if __name__ == "__main__":
    ...
    weaver_private, weaver_public = generate_keypair("ECHO")
    partner_private, partner_public = generate_keypair("PARTNER")

    # Step 2: Sign a message from ECHO
    print("\n[Step 2] ECHO signing a message...")
    message = {"action": "hello", "content": "Greetings from ECHO!"}
    headers = sign_message(message, str(weaver_private), "ECHO")
    print(f"  Message: {message}")
    print(f"  Headers: {headers}")

    # Step 3: PARTNER verifies the signature
    print("\n[Step 3] PARTNER verifying ECHO's signature...")
...
class TestPersonaCompletion:
    ...
    def test_complete_tutorial_under_15_minutes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            weaver_priv, weaver_pub = generate_keypair("ECHO", tmpdir)
            partner_priv, partner_pub = generate_keypair("PARTNER", tmpdir)

            # Section 2: Sign a message
            message = {"greeting": "Hello from ECHO"}
            headers = sign_message(message, str(weaver_priv), "ECHO")
            ...
            # ECHO verifies PARTNER's message
