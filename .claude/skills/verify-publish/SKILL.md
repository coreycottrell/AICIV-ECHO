...
description: |
  ...
  prepares LinkedIn for Corey (or auto-posts if enabled).
version: 1.0.0
...
This is the quality gate before publication. Every factual claim gets verified. Every source gets checked. Then we publish to blog, post to Bluesky, and prepare LinkedIn for Corey.
...
```bash
cat /mnt/c/Users/100kp/projects/AI-CIV/ECHO/exports/daily-pipeline/$(date +%Y-%m-%d)/blog-post.md
cat /mnt/c/Users/100kp/projects/AI-CIV/ECHO/exports/daily-pipeline/$(date +%Y-%m-%d)/metadata.json
```
...
**USE sageandweaver-blog SKILL** - ECHO publishes its own blog via Netlify:
...
cp exports/blog-header.png \
   /mnt/c/Users/100kp/projects/AI-CIV/ACG/sageandweaver-network/weaver-blog/images/

# 3. Update posts.json (add entry at TOP)

# 4. Deploy to Netlify
cd /mnt/c/Users/100kp/projects/AI-CIV/ACG/sageandweaver-network && \
...
**DO NOT send to hub. ECHO publishes its OWN blog.**
...
```bash
# Email to Corey for manual posting
python3 /mnt/c/Users/100kp/projects/AI-CIV/ECHO/tools/send_email.py \
...
# NOT YET IMPLEMENTED - requires Corey approval
```

#### 6d: Send Image to Corey

If image generation is part of pipeline:
```bash
python3 /mnt/c/Users/100kp/projects/AI-CIV/ECHO/tools/send_telegram_file.sh \
    437939400 \
...
```
/mnt/c/Users/100kp/projects/AI-CIV/ECHO/exports/daily-pipeline/YYYY-MM-DD/publication-log.md
```
...
### LinkedIn
- **Status**: [Sent to Corey / Auto-posted]
...
- [ ] LinkedIn post sent to Corey (or auto-posted)
...
1. Try API deploy: `python3 /mnt/c/Users/100kp/projects/AI-CIV/ECHO/tools/netlify_api_deploy.py`
2. If still fails, check netlify token in ~/.config/netlify/config.json
3. Email blog to Corey as backup
...
- Published by ECHO directly via Netlify
...
Add to `/mnt/c/Users/100kp/projects/AI-CIV/ECHO/tools/daily_pipeline_cron.sh`:
